				...
				<div class="luwi-orders__meta">
					<span class="luwi-orders__count">
						...
						printf(
							...
							esc_html( _n( '%d item', '%d items', $item_count, 'luwi-ruby' ) ),
							absint( $item_count )
						);
						?>
					</span>
	...
	<?php if ( 1 < $customer_orders->max_num_pages ) : ?>
		<div class="woocommerce-pagination woocommerce-pagination--without-numbers woocommerce-Pagination">
			<?php if ( 1 !== $current_page ) : ?>
				<a class="woocommerce-button woocommerce-button--previous woocommerce-Button woocommerce-Button--previous button<?php echo esc_attr( wc_wp_theme_get_element_class_name( 'button' ) ? ' ' . wc_wp_theme_get_element_class_name( 'button' ) : '' ); ?>" href="<?php echo esc_url( wc_get_endpoint_url( 'orders', $current_page - 1 ) ); ?>"><?php esc_html_e( 'Previous', 'luwi-ruby' ); ?></a>
			<?php endif; ?>

			<?php if ( intval( $customer_orders->max_num_pages ) !== $current_page ) : ?>
				<a class="woocommerce-button woocommerce-button--next woocommerce-Button woocommerce-Button--next button<?php echo esc_attr( wc_wp_theme_get_element_class_name( 'button' ) ? ' ' . wc_wp_theme_get_element_class_name( 'button' ) : '' ); ?>" href="<?php echo esc_url( wc_get_endpoint_url( 'orders', $current_page + 1 ) ); ?>"><?php esc_html_e( 'Next', 'luwi-ruby' ); ?></a>
			<?php endif; ?>
		</div>
	<?php endif; ?>
...
<?php else : ?>
	...
	<div class="luwi-empty-state">
		...
		<h2 class="luwi-empty-state__title"><?php esc_html_e( 'No orders yet', 'luwi-ruby' ); ?></h2>
		<p class="luwi-empty-state__text"><?php esc_html_e( 'Browse our collection and place your first order.', 'luwi-ruby' ); ?></p>
		<a class="luwi-btn luwi-btn--primary" href="<?php echo esc_url( apply_filters( 'woocommerce_return_to_shop_redirect', wc_get_page_permalink( 'shop' ) ) ); ?>">
			<?php esc_html_e( 'Start Shopping', 'luwi-ruby' ); ?>
		</a>
	</div>
