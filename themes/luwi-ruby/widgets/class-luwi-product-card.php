...
class Luwi_Widget_Product_Card extends Luwi_Widget_Base {
	...
	public function get_title() {
		return __( 'Luwi Product Card', 'luwi-ruby' );
	}
	...
	protected function register_controls() {
		...
		$this->add_control(
			'source',
			array(
				'label'   => __( 'Source', 'luwi-ruby' ),
				'type'    => \Elementor\Controls_Manager::SELECT,
				'default' => 'latest',
				'options' => array(
					'latest'   => __( 'Latest Products', 'luwi-ruby' ),
					'featured' => __( 'Featured', 'luwi-ruby' ),
					'sale'     => __( 'On Sale', 'luwi-ruby' ),
					'best'     => __( 'Best Selling', 'luwi-ruby' ),
					'category' => __( 'By Category', 'luwi-ruby' ),
				),
			)
		);
		...
		$this->add_control(
			'category',
			array(
				'label'     => __( 'Category', 'luwi-ruby' ),
				'type'      => \Elementor\Controls_Manager::TEXT,
				'default'   => '',
				'condition' => array( 'source' => 'category' ),
				'description' => __( 'Enter category slug', 'luwi-ruby' ),
			)
		);
		...
		$this->add_control(
			'count',
			array(
				'label'   => __( 'Number of Products', 'luwi-ruby' ),
		...
		$this->add_control(
			'card_style',
			array(
				'label'   => __( 'Card Style', 'luwi-ruby' ),
				'type'    => \Elementor\Controls_Manager::SELECT,
				'default' => 'below',
				'options' => array(
					'below'   => __( 'Title Below Image', 'luwi-ruby' ),
					'overlay' => __( 'Image with Overlay', 'luwi-ruby' ),
				),
			)
		);
		...
		$this->add_control(
			'show_badge',
			array(
				'label'        => __( 'Show Sale Badge', 'luwi-ruby' ),
		...
		$this->add_control(
			'show_category',
			array(
				'label'        => __( 'Show Category', 'luwi-ruby' ),
				'type'         => \Elementor\Controls_Manager::SWITCHER,
				'default'      => 'yes',
				'return_value' => 'yes',
			)
		...
		$this->add_control(
			'show_rating',
			array(
				'label'        => __( 'Show Rating', 'luwi-ruby' ),
				'type'         => \Elementor\Controls_Manager::SWITCHER,
				'default'      => '',
				'return_value' => 'yes',
			)
		...
		$this->add_control(
			'show_cta',
			array(
				'label'        => __( 'Show Add to Cart Button', 'luwi-ruby' ),
				'type'         => \Elementor\Controls_Manager::SWITCHER,
				'default'      => 'yes',
				'return_value' => 'yes',
			)
		...
		$this->add_control(
			'cta_text',
			array(
				'label'     => __( 'Button Text', 'luwi-ruby' ),
				'type'      => \Elementor\Controls_Manager::TEXT,
				'default'   => __( 'Add to Cart', 'luwi-ruby' ),
		...
		$this->add_control(
			'show_quick_view',
			array(
				'label'        => __( 'Show Quick View on Hover', 'luwi-ruby' ),
				'type'         => \Elementor\Controls_Manager::SWITCHER,
				'default'      => '',
				'return_value' => 'yes',
			)
		...
		$this->add_control(
			'card_bg',
			array(
				'label'     => __( 'Card Background', 'luwi-ruby' ),
		...
		$this->add_responsive_control(
			'card_padding',
			array(
				'label'      => __( 'Card Body Padding', 'luwi-ruby' ),
	...
	protected function render() {
		if ( ! function_exists( 'wc_get_products' ) ) {
			if ( \Elementor\Plugin::$instance->editor->is_edit_mode() ) {
				...
				echo esc_html__( 'WooCommerce is required for this widget.', 'luwi-ruby' );
				echo '</div>';
			}
			return;
		}
		...
		if ( empty( $products ) ) {
			if ( \Elementor\Plugin::$instance->editor->is_edit_mode() ) {
				echo '<div class="luwi-widget-placeholder">';
				echo esc_html__( 'No products found. Add products or change the query settings.', 'luwi-ruby' );
				echo '</div>';
			}
			return;
		}
		...
		$this->render_luwipress_upsell(
			__( 'With LuwiPress: AI-recommended products from Knowledge Graph.', 'luwi-ruby' )
		);
	}
	...
	private function render_product_card( $product, $settings ) {
		...
		if ( $show_badge && $product->is_on_sale() ) {
			...
			if ( $regular > 0 && $sale > 0 ) {
				...
				echo '<span class="luwi-product-card__badge">' . esc_html__( 'Sale', 'luwi-ruby' ) . '</span>';
			}
		}
		...
		if ( $show_qv ) {
			...
			printf(
				...
				esc_attr__( 'Quick view', 'luwi-ruby' ),
				esc_html__( 'Quick View', 'luwi-ruby' )
			);
			echo '</div>';
		}
		...
		if ( $show_cta && $product->is_purchasable() && $product->is_in_stock() ) {
			$cta_text = ! empty( $settings['cta_text'] ) ? $settings['cta_text'] : __( 'Add to Cart', 'luwi-ruby' );
			printf(
				...
				esc_attr( sprintf( __( 'Add "%s" to cart', 'luwi-ruby' ), $product->get_name() ) ),
				esc_html( $cta_text )
			);
		}
