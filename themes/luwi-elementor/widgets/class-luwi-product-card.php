...
class Luwi_Widget_Product_Card extends Luwi_Widget_Base {
	...
	public function get_title() {
		return __( 'Luwi Product Card', 'luwi-elementor' );
	}
	...
	protected function register_controls() {
		...
		$this->add_control(
			'source',
			array(
				'label'   => __( 'Source', 'luwi-elementor' ),
				'type'    => \Elementor\Controls_Manager::SELECT,
				'default' => 'latest',
				'options' => array(
					'latest'   => __( 'Latest Products', 'luwi-elementor' ),
					'featured' => __( 'Featured', 'luwi-elementor' ),
					'sale'     => __( 'On Sale', 'luwi-elementor' ),
					'best'     => __( 'Best Selling', 'luwi-elementor' ),
					'category' => __( 'By Category', 'luwi-elementor' ),
				),
			)
		);
		...
		$this->add_control(
			'category',
			array(
				'label'     => __( 'Category', 'luwi-elementor' ),
				'type'      => \Elementor\Controls_Manager::TEXT,
				'default'   => '',
				'condition' => array( 'source' => 'category' ),
				'description' => __( 'Enter category slug', 'luwi-elementor' ),
			)
		);
		...
		$this->add_control(
			'count',
			array(
				'label'   => __( 'Number of Products', 'luwi-elementor' ),
		...
		$this->add_responsive_control(
			'columns',
			array(
				'label'   => __( 'Columns', 'luwi-elementor' ),
				'type'    => \Elementor\Controls_Manager::SELECT,
				'default' => '3',
				'options' => array(
					'1' => '1',
		...
		$this->add_control(
			'card_style',
			array(
				'label'   => __( 'Card Style', 'luwi-elementor' ),
				'type'    => \Elementor\Controls_Manager::SELECT,
				'default' => 'below',
				'options' => array(
					'below'   => __( 'Title Below Image', 'luwi-elementor' ),
					'overlay' => __( 'Image with Overlay', 'luwi-elementor' ),
				),
			)
		);
		...
		$this->add_control(
			'show_badge',
			array(
				'label'        => __( 'Show Sale Badge', 'luwi-elementor' ),
		...
		$this->add_control(
			'show_category',
			array(
				'label'        => __( 'Show Category', 'luwi-elementor' ),
				'type'         => \Elementor\Controls_Manager::SWITCHER,
				'default'      => 'yes',
				'return_value' => 'yes',
			)
		...
		$this->add_control(
			'show_rating',
			array(
				'label'        => __( 'Show Rating', 'luwi-elementor' ),
				'type'         => \Elementor\Controls_Manager::SWITCHER,
				'default'      => '',
				'return_value' => 'yes',
			)
		...
		$this->add_control(
			'show_cta',
			array(
				'label'        => __( 'Show Add to Cart Button', 'luwi-elementor' ),
				'type'         => \Elementor\Controls_Manager::SWITCHER,
				'default'      => 'yes',
				'return_value' => 'yes',
			)
		...
		$this->add_control(
			'cta_text',
			array(
				'label'     => __( 'Button Text', 'luwi-elementor' ),
				'type'      => \Elementor\Controls_Manager::TEXT,
				'default'   => __( 'Add to Cart', 'luwi-elementor' ),
		...
		$this->add_control(
			'show_quick_view',
			array(
				'label'        => __( 'Show Quick View on Hover', 'luwi-elementor' ),
				'type'         => \Elementor\Controls_Manager::SWITCHER,
				'default'      => '',
				'return_value' => 'yes',
			)
		...
		$this->add_control(
			'card_bg',
			array(
				'label'     => __( 'Card Background', 'luwi-elementor' ),
		...
		$this->add_responsive_control(
			'card_padding',
			array(
				'label'      => __( 'Card Body Padding', 'luwi-elementor' ),
		...
		$this->add_responsive_control(
			'grid_gap',
			array(
				'label'      => __( 'Grid Gap', 'luwi-elementor' ),
	...
	protected function render() {
		if ( ! function_exists( 'wc_get_products' ) ) {
			if ( \Elementor\Plugin::$instance->editor->is_edit_mode() ) {
				...
				echo esc_html__( 'WooCommerce is required for this widget.', 'luwi-elementor' );
				echo '</div>';
			}
			return;
		}
		...
		if ( empty( $products ) ) {
			if ( \Elementor\Plugin::$instance->editor->is_edit_mode() ) {
				echo '<div class="luwi-widget-placeholder">';
				echo esc_html__( 'No products found. Add products or change the query settings.', 'luwi-elementor' );
				echo '</div>';
			}
			return;
		}
		...
		$this->render_luwipress_upsell(
			__( 'With LuwiPress: AI-recommended products from Knowledge Graph.', 'luwi-elementor' )
		);
	}
	...
	private function render_product_card( $product, $settings ) {
		...
		if ( $show_qv ) {
			...
			printf(
				...
				esc_attr__( 'Quick view', 'luwi-elementor' ),
				esc_html__( 'Quick View', 'luwi-elementor' )
			);
			echo '</div>';
		}
		...
		if ( $show_cta && $product->is_purchasable() && $product->is_in_stock() ) {
			$cta_text = ! empty( $settings['cta_text'] ) ? $settings['cta_text'] : __( 'Add to Cart', 'luwi-elementor' );
			printf(
				...
				esc_attr( sprintf( __( 'Add "%s" to cart', 'luwi-elementor' ), $product->get_name() ) ),
				esc_html( $cta_text )
			);
		}
