...
class LuwiPress_API {
    ...
    public function register_endpoints() {
        ...
        register_rest_route('luwipress/v1', '/webhook', array(
            'methods' => 'POST',
        ...
        register_rest_route( 'luwipress/v1', '/token-stats', array(
            'methods'             => 'GET',
        ...
        register_rest_route( 'luwipress/v1', '/token-recent', array(
            'methods'             => 'GET',
        ...
        // Status endpoint
        register_rest_route('luwipress/v1', '/status', array(
            'methods' => 'GET',
        ...
        register_rest_route('luwipress/v1', '/health', array(
            'methods' => 'GET',
        ...
        register_rest_route('luwipress/v1', '/test/rank-math', array(
            'methods'             => 'GET',
        ...
        register_rest_route('luwipress/v1', '/test/aeo-coverage', array(
            'methods'             => 'GET',
        ...
        register_rest_route( 'luwipress/v1', '/seo/meta', array(
            'methods'             => 'POST',
    ...
    private function check_rate_limit() {
        ...
        $limit = luwipress_get_option('rate_limit', 1000);
        
        $transient_key = 'luwipress_rate_limit_' . md5($ip);
        ...
        if ($requests >= $limit) {
            LuwiPress_Logger::log("Rate limit exceeded for IP: $ip", 'warning');
            return false;
        }
    ...
    private function check_ip_whitelist() {
        $whitelist = luwipress_get_option('ip_whitelist', '');
    ...
    public function handle_webhook($request) {
        ...
        try {
            ...
            // Log request
            LuwiPress_Logger::log('Webhook request received', 'info', array(
            ...
            // Log success
            LuwiPress_Logger::log('Webhook processed successfully', 'info', array(
                'action' => $data['action'],
                'request_id' => $data['request_id'],
            ...
            LuwiPress_Logger::log('Webhook error: ' . $e->getMessage(), 'error', array(
    ...
    private function authenticate_request($data) {
        ...
        return LuwiPress_Auth::validate_token($data['auth_token']);
    }
    ...
    private function create_post($data, $user) {
        ...
        // Add request metadata
        $post_data['meta_input']['_luwipress_request_id'] = $data['request_id'];
        $post_data['meta_input']['_luwipress_created_via'] = 'webhook';
    ...
    private function update_post($data, $user) {
        ...
        // Update metadata
        update_post_meta($post_id, '_luwipress_last_updated_via', 'webhook');
        update_post_meta($post_id, '_luwipress_last_request_id', $data['request_id']);
    ...
    public function get_status() {
        ...
        if ( current_user_can( 'manage_options' ) ) {
            ...
            $response['endpoints'] = array(
                'webhook' => rest_url('luwipress/v1/webhook'),
                'status'  => rest_url('luwipress/v1/status'),
                'health'  => rest_url('luwipress/v1/health'),
            );
            ...
            $response['features'] = array(
                ...
                'logging_enabled' => luwipress_get_option('enable_logging', 1),
                'rate_limiting'   => luwipress_get_option('rate_limit', 1000) > 0,
            );
        }
    ...
    /**
     * Permission check: API token via Authorization header or X-LuwiPress-Token
     */
    ...
    public function handle_workflow_result( $request ) {
        ...
        if ( ! empty( $token_data ) && class_exists( 'LuwiPress_Token_Tracker' ) ) {
    ...
    public function handle_get_token_usage( $request ) {
        ...
        return rest_ensure_response( LuwiPress_Token_Tracker::get_stats( $days ) );
    }
    ...
    public function handle_check_limit( $request ) {
        $exceeded = class_exists( 'LuwiPress_Token_Tracker' ) && LuwiPress_Token_Tracker::is_limit_exceeded();
        $today    = class_exists( 'LuwiPress_Token_Tracker' ) ? LuwiPress_Token_Tracker::get_today_cost() : 0;
    ...
    public function handle_token_usage_report( $request ) {
        ...
        $data = $request->get_json_params();
        $cost = LuwiPress_Token_Tracker::record( array(
            'workflow'      => sanitize_text_field( $data['workflow'] ?? 'unknown' ),
    ...
    public function handle_get_token_stats( $request ) {
        ...
        return rest_ensure_response( LuwiPress_Token_Tracker::get_stats( 30 ) );
    }
    ...
    public function test_rank_math_meta( $request ) {
        ...
        return rest_ensure_response( array(
            ...
            'meta'             => $meta,
            'luwipress_schema'  => (bool) get_post_meta( $post_id, '_luwipress_schema', true ),
            'luwipress_faq'     => count( (array) get_post_meta( $post_id, '_luwipress_faq', true ) ),
            'luwipress_howto'   => (bool) get_post_meta( $post_id, '_luwipress_howto', true ),
        ) );
    }

    /**
     * GET /test/aeo-coverage
     * Delegates to LuwiPress_AEO::get_aeo_coverage().
     */
     ...
     * Returns recent log entries from the LuwiPress log table.
     */
    public function get_recent_logs( $request ) {
        ...
        $logs = LuwiPress_Logger::get_logs( $limit, $level );
    ...
    public function handle_set_seo_meta( $request ) {
        ...
        LuwiPress_Logger::log( 'SEO meta updated via API', 'info', array(
            'post_id' => $post_id,
