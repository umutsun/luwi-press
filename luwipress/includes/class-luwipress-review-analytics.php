...
class LuwiPress_Review_Analytics {
	...
	public function register_endpoints() {
		...
		register_rest_route( 'luwipress/v1', '/review/analytics', array(
			'methods'             => 'GET',
		...
		register_rest_route( 'luwipress/v1', '/review/summary', array(
			'methods'             => 'GET',
	...
	public function handle_sentiment_callback( $request ) {
		...
		update_comment_meta( $review_id, '_luwipress_sentiment', $review_data );

		// Update product aggregate
		$this->update_product_aggregate( $product_id );

		LuwiPress_Logger::log( 'Review sentiment saved', 'info', array(
			'review_id'  => $review_id,
	...
	public function handle_get_summary( $request ) {
		...
		$sentiments = $wpdb->get_results(
			"SELECT cm.meta_value
			 ...
			 WHERE cm.meta_key = '_luwipress_sentiment'
	...
	public function output_aggregate_rating_schema() {
		...
		echo "\n<!-- LuwiPress AggregateRating Schema -->\n";
	...
	private function update_product_aggregate( $product_id ) {
		...
		foreach ( $reviews as $review ) {
			$data = get_comment_meta( $review->comment_ID, '_luwipress_sentiment', true );
			if ( ! is_array( $data ) ) {
				continue;
			}
		...
		update_post_meta( $product_id, '_luwipress_review_aggregate', $aggregate );
	}
	...
	private function get_attention_needed() {
		...
		foreach ( $results as $row ) {
			$aggregate = get_post_meta( $row->product_id, '_luwipress_review_aggregate', true );
			$products[] = array(
