<?php
/**
 * LuwiPress Authentication Handler
 */
...
class LuwiPress_Auth {
    ...
    public function generate_token($request) {
        ...
        LuwiPress_Logger::log('JWT token generated', 'info', array(
            'user_id' => $user->ID,
    ...
    public function refresh_token($request) {
        ...
        LuwiPress_Logger::log('JWT token refreshed', 'info', array(
            'user_id' => $user->ID,
            'expires' => $token_data['expires']
        ));
    ...
    public static function validate_token($token, $check_expiration = true) {
        ...
        try {
            ...
            LuwiPress_Logger::log('JWT validation failed: ' . $e->getMessage(), 'debug');
    ...
    private function create_jwt_token($user) {
        $secret_key = self::get_secret_key();
        $issued_at = time();
        $expires_in = apply_filters('luwipress_jwt_expires_in', WEEK_IN_SECONDS);
        ...
        // Add custom claims
        $payload = apply_filters('luwipress_jwt_payload', $payload, $user);
    ...
    public static function get_secret_key() {
        ...
        if (!$secret) {
            $secret = get_option('luwipress_jwt_secret');
        }
        
        if (!$secret) {
            // Generate a secret if none exists
            $secret = base64_encode(random_bytes(32));
            update_option('luwipress_jwt_secret', $secret);
        }
    ...
    public function add_custom_claims($token, $user) {
        // Add LuwiPress specific claims
        $token['luwipress_version'] = LUWIPRESS_VERSION;
        $token['issued_by'] = 'luwipress';
        
        return $token;
    }
    
    public static function create_application_password($user_id, $app_name = 'LuwiPress') {
    ...
    public static function revoke_token( $jti ) {
        $revoked = get_option( 'luwipress_revoked_tokens', array() );
        $revoked[ $jti ] = time();
        ...
        update_option( 'luwipress_revoked_tokens', $revoked, 'no' );
    }
