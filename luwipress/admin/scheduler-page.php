 ...
 *
 * AI-powered content generation and scheduling interface (wizard).
 *
 * @since 1.10.0
 */
...
// Translation plugin detection → drives the multilingual panel in Step 2
...
<div class="wrap luwipress-dashboard">
	...
	<div class="sched-stats">
		...
		?>
		<div class="sched-stat-card sched-stat--<?php echo esc_attr( $cfg['accent'] ); ?>">
	...
	<div class="lp-card sched-wizard-card">
		<div class="lp-card-header sched-wiz-header">
			<h3><span class="dashicons dashicons-welcome-write-blog"></span> <?php esc_html_e( 'Create & Queue', 'luwipress' ); ?></h3>
			<ol class="sched-steps" id="sched-steps">
				<li class="sched-step is-active" data-step="1"><span class="sched-step-num">1</span><span class="sched-step-lbl"><?php esc_html_e( 'Topics', 'luwipress' ); ?></span></li>
				<li class="sched-step" data-step="2"><span class="sched-step-num">2</span><span class="sched-step-lbl"><?php esc_html_e( 'Style', 'luwipress' ); ?></span></li>
				<li class="sched-step" data-step="3"><span class="sched-step-num">3</span><span class="sched-step-lbl"><?php esc_html_e( 'Schedule', 'luwipress' ); ?></span></li>
				<li class="sched-step" data-step="4"><span class="sched-step-num">4</span><span class="sched-step-lbl"><?php esc_html_e( 'Review', 'luwipress' ); ?></span></li>
			</ol>
		</div>

		<form id="sched-wizard-form" autocomplete="off">
			...
			<section class="sched-wiz-panel is-active" data-panel="1">
				<div class="sched-wiz-intro">
					<h4><?php esc_html_e( 'What do you want to publish?', 'luwipress' ); ?></h4>
					<p><?php esc_html_e( 'Add one topic per line. You can queue up to 50 in a single pass. Optionally add comma-separated keywords after a pipe — e.g. "History of the santur | ethnic instruments, turkish music".', 'luwipress' ); ?></p>
				</div>
				<div class="sched-field">
					<div class="sched-field-head">
						<label for="sched-wiz-topics"><?php esc_html_e( 'Topics', 'luwipress' ); ?> <span class="sched-req">*</span></label>
						<span class="sched-wiz-head-actions">
				...
				<!-- Topic brainstorm panel (collapsible, inline) -->
				<div class="sched-brainstorm" id="sched-brainstorm" hidden>
					<div class="sched-brainstorm-head">
						<h5><span class="dashicons dashicons-lightbulb"></span> <?php esc_html_e( 'AI topic brainstorm', 'luwipress' ); ?></h5>
						<p><?php esc_html_e( 'Give the AI a theme. It proposes specific, publishable titles (not generic lists) — filtering against your last 30 post titles so it doesn\'t suggest duplicates.', 'luwipress' ); ?></p>
					</div>
					<div class="sched-brainstorm-form">
						<div class="sched-brainstorm-fields">
							<div class="sched-field">
								...
								<input type="text" id="sched-brainstorm-theme" placeholder="<?php esc_attr_e( 'e.g. product care basics, buyer guides, craft stories — be specific', 'luwipress' ); ?>" />
							</div>
							...
							<div class="sched-field">
								...
								<input type="text" id="sched-brainstorm-style" placeholder="<?php esc_attr_e( 'e.g. story-driven, technique-focused, comparison, buyer-journey', 'luwipress' ); ?>" />
							</div>
						</div>
				...
				<details class="sched-wiz-syntax">
					...
					<div class="sched-wiz-syntax-body">
						<p><?php esc_html_e( 'You can override batch defaults for individual rows using pipe-separated key=value segments:', 'luwipress' ); ?></p>
						<pre class="sched-wiz-code">Long flagship topic title | depth=editorial | words=3000
Short product care topic | depth=standard | words=900 | image=0
Topic with SEO focus | keywords=primary term, secondary term | tone=creative</pre>
						<p class="sched-wiz-syntax-keys"><?php esc_html_e( 'Keys:', 'luwipress' ); ?> <code>keywords</code> · <code>depth</code> (standard/deep/editorial) · <code>words</code> (300–5000) · <code>tone</code> · <code>lang</code> · <code>image</code> (0/1) · <code>type</code></p>
					</div>
				</details>
			</section>
			...
			<section class="sched-wiz-panel" data-panel="2" hidden>
				<div class="sched-wiz-intro">
					<h4><?php esc_html_e( 'Pick the voice and depth', 'luwipress' ); ?></h4>
				...
				<!-- Outline approval gate (deep/editorial only; no-op for standard) -->
				<label class="sched-checkbox sched-outline-gate" id="sched-outline-gate">
					...
					<span>
						...
						<small><?php esc_html_e( 'Two-phase flow: AI drafts a structural outline first and waits for your edits. Only applies to deep/editorial depth — standard skips this step.', 'luwipress' ); ?></small>
					</span>
				</label>
				...
				<div class="sched-field sched-i18n-field" id="sched-i18n-field">
					...
					<p class="sched-i18n-help"><?php esc_html_e( 'Each selected language gets its own article written natively in that language (not a machine translation) and linked to the primary via the translation plugin. AI cost multiplies per language.', 'luwipress' ); ?></p>
				...
				<details class="sched-wiz-voice" <?php echo '' !== $brand_voice_default ? 'open' : ''; ?>>
					...
					<div class="sched-wiz-voice-body">
						...
						<textarea name="brand_voice" rows="6" class="large-text sched-wiz-voice-text" placeholder="<?php esc_attr_e( "Example:\n• Audience profile (who's reading, what they already know).\n• Desired register — curator, practitioner, teacher, journalist, storyteller.\n• Forbidden openers / clichés (e.g. 'in today's fast-paced world', 'as we all know').\n• Preferred opening style (anecdote, contrast, specific fact, quote).\n• Cultural, regional, or historical context to weave in when relevant.\n• Words, claims, or competitor mentions to avoid entirely.", 'luwipress' ); ?>"><?php echo esc_textarea( $brand_voice_default ); ?></textarea>
			...
			<section class="sched-wiz-panel" data-panel="3" hidden>
				<div class="sched-wiz-intro">
					<h4><?php esc_html_e( 'When and how should these publish?', 'luwipress' ); ?></h4>
				...
				<div class="sched-field">
					<label><?php esc_html_e( 'After AI generates each article', 'luwipress' ); ?></label>
					<div class="sched-mode-grid">
						<label class="sched-mode-card">
							...
							<div class="sched-mode-inner">
								...
								<small><?php esc_html_e( 'Ships straight into WP drafts with the target publish date baked in. Review, enrich, then hit Publish yourself.', 'luwipress' ); ?></small>
							</div>
						</label>
						<label class="sched-mode-card">
				...
				<div class="sched-row-2">
					<div class="sched-field">
						<label for="sched-wiz-date"><?php esc_html_e( 'Start date', 'luwipress' ); ?> <span class="sched-req">*</span></label>
			...
			<section class="sched-wiz-panel" data-panel="4" hidden>
				<div class="sched-wiz-intro">
					<h4><?php esc_html_e( 'Ready to queue?', 'luwipress' ); ?></h4>
				...
				<div class="sched-wiz-budget" id="sched-wiz-budget" hidden>
					...
					<p class="sched-wiz-budget-disclaimer"><?php esc_html_e( 'Estimate based on current provider pricing and per-article token heuristics. Actual cost may vary ±20% depending on output length. Image cost is approximate (DALL·E 3 std).', 'luwipress' ); ?></p>
				</div>
			</section>
			...
			<div class="sched-wiz-nav">
				...
				<div class="sched-wiz-progress" aria-hidden="true">
		...
		<div id="sched-wiz-result"></div>
	</div>

	<!-- Queue List -->
	<div class="lp-card sched-queue-card">
		<div class="lp-card-header">
			<h3><span class="dashicons dashicons-list-view"></span> <?php esc_html_e( 'Queue', 'luwipress' ); ?></h3>
		...
		<div class="sched-empty">
			...
			<p><?php esc_html_e( 'Queue your first batch using the wizard above.', 'luwipress' ); ?></p>
		</div>
		<?php else :
			// Filter tabs
			$filters = array(
		...
		?>
		<div class="sched-queue-filters" id="sched-queue-filters">
			<?php foreach ( $filters as $key => $label ) :
				if ( 'all' !== $key && 0 === $counts[ $key ] ) continue;
				$n = 'all' === $key ? count( $scheduled_items ) : $counts[ $key ];
			?>
			<button type="button" class="sched-filter <?php echo 'all' === $key ? 'is-active' : ''; ?>" data-filter="<?php echo esc_attr( $key ); ?>">
				<?php echo esc_html( $label ); ?> <span class="sched-filter-count"><?php echo absint( $n ); ?></span>
			</button>
			<?php endforeach; ?>
		</div>
		...
		<div class="sched-list" id="sched-list">
			<?php foreach ( $scheduled_items as $item ) :
				...
				// Only check post_status for draft-mode items — auto-mode always creates published posts.
			...
			<div class="sched-item <?php echo 'generating' === $status ? 'sched-pulse' : ''; ?>" data-id="<?php echo absint( $item->ID ); ?>" data-status="<?php echo esc_attr( $status ); ?>" data-post-status="<?php echo esc_attr( $published_id && $is_draft ? 'draft' : ( $published_id ? 'publish' : '' ) ); ?>">
				...
				<div class="sched-item-status sched-item-status--<?php echo esc_attr( $cfg['accent'] ); ?>">
					<span class="dashicons dashicons-<?php echo esc_attr( $cfg['icon'] ); ?>"></span>
				</div>
				...
				<div class="sched-item-actions">
					...
					<button type="button" class="button button-small sched-delete" data-id="<?php echo absint( $item->ID ); ?>" title="<?php esc_attr_e( 'Discard', 'luwipress' ); ?>">
					...
					<?php else : ?>
						...
						<button type="button" class="button button-small sched-retry" data-id="<?php echo absint( $item->ID ); ?>" title="<?php esc_attr_e( 'Retry', 'luwipress' ); ?>">
							<span class="dashicons dashicons-update"></span> <?php esc_html_e( 'Retry', 'luwipress' ); ?>
						</button>
						<?php endif; ?>
						<?php if ( 'published' !== $status ) : ?>
						<button type="button" class="button button-small sched-delete" data-id="<?php echo absint( $item->ID ); ?>" title="<?php esc_attr_e( 'Delete', 'luwipress' ); ?>">
							<span class="dashicons dashicons-trash"></span>
						</button>
						<?php endif; ?>
					<?php endif; ?>
	...
	<!-- Recurring plans -->
	<?php $recurring_plans = LuwiPress_Content_Scheduler::get_recurring_plans(); ?>
	<div class="lp-card sched-plans-card">
		<div class="lp-card-header">
			<h3><span class="dashicons dashicons-backup"></span> <?php esc_html_e( 'Recurring plans', 'luwipress' ); ?></h3>
			<div class="sched-queue-actions">
				<span class="lp-card-badge"><?php echo count( $recurring_plans ); ?> <?php esc_html_e( 'plans', 'luwipress' ); ?></span>
				<button type="button" class="button button-primary sched-plan-new" id="sched-plan-new">
					<span class="dashicons dashicons-plus-alt"></span> <?php esc_html_e( 'New plan', 'luwipress' ); ?>
				</button>
			</div>
		</div>

		<p class="sched-plans-help">
			<?php esc_html_e( "Set a theme and a cadence. LuwiPress brainstorms fresh topics for you on schedule, filtering against your recent posts so you don't get duplicates. Each run respects your daily AI budget — hit the cap and the plan defers to the next tick.", 'luwipress' ); ?>
		</p>

		<?php if ( empty( $recurring_plans ) ) : ?>
		<div class="sched-plans-empty">
			<span class="dashicons dashicons-backup"></span>
			<h3><?php esc_html_e( 'No recurring plans yet', 'luwipress' ); ?></h3>
			<p><?php esc_html_e( 'Create one to keep your editorial calendar filling itself.', 'luwipress' ); ?></p>
		</div>
		<?php else : ?>
			...
			<div class="sched-plan-row <?php echo $paused ? 'is-paused' : ''; ?>" data-id="<?php echo esc_attr( $plan['id'] ); ?>">
				<div class="sched-plan-main">
					...
					<div class="sched-plan-dates">
						<span><strong><?php esc_html_e( 'Next run:', 'luwipress' ); ?></strong> <?php echo esc_html( $next ); ?></span>
						<span><strong><?php esc_html_e( 'Last run:', 'luwipress' ); ?></strong> <?php echo esc_html( $last ); ?></span>
				...
				<div class="sched-plan-actions">
					...
					<button type="button" class="button button-small sched-plan-edit" data-id="<?php echo esc_attr( $plan['id'] ); ?>" data-plan='<?php echo esc_attr( wp_json_encode( $plan ) ); ?>'>
						<span class="dashicons dashicons-edit"></span> <?php esc_html_e( 'Edit', 'luwipress' ); ?>
					</button>
					<button type="button" class="button button-small sched-plan-delete" data-id="<?php echo esc_attr( $plan['id'] ); ?>">
						<span class="dashicons dashicons-trash"></span>
					</button>
				</div>
			</div>
	...
	<div class="sched-modal" id="sched-plan-modal" hidden aria-hidden="true" role="dialog" aria-modal="true">
		...
		<div class="sched-modal-panel">
			<header class="sched-modal-head">
				...
				<button type="button" class="sched-modal-close" data-close="1"><span class="dashicons dashicons-no-alt"></span></button>
			</header>
			<div class="sched-modal-body">
				<form id="sched-plan-form" autocomplete="off">
					...
					<div class="sched-row-2">
						<div class="sched-field">
							<label><?php esc_html_e( 'Plan name', 'luwipress' ); ?> <span class="sched-req">*</span></label>
					...
					<div class="sched-field">
						<label><?php esc_html_e( 'Theme', 'luwipress' ); ?> <span class="sched-req">*</span></label>
