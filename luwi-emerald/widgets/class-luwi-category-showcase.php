...
class Luwi_Widget_Category_Showcase extends Luwi_Widget_Base {
	...
	public function get_title() {
		return __( 'Luwi Category Showcase', 'luwi-emerald' );
	}
	...
	protected function register_controls() {
		...
		$this->start_controls_section(
			'content_section',
			array(
				'label' => __( 'Categories', 'luwi-emerald' ),
		...
		$this->add_control(
			...
			array(
				...
				'default' => 'auto',
				'options' => array(
					'auto'   => __( 'Auto (Top WooCommerce Categories)', 'luwi-emerald' ),
					'manual' => __( 'Manual Selection', 'luwi-emerald' ),
				),
			)
		);
		...
		$this->add_control(
			'count',
			array(
				'label'     => __( 'Number of Categories', 'luwi-emerald' ),
		...
		$this->add_control(
			'show_count',
			array(
				'label'        => __( 'Show Product Count', 'luwi-emerald' ),
		...
		$this->add_control(
			...
			array(
				...
				'options' => array(
					...
					'minimal' => __( 'Minimal (Icon + Text)', 'luwi-emerald' ),
				),
			)
		);
	...
	protected function render() {
		...
		if ( empty( $categories ) ) {
			if ( \Elementor\Plugin::$instance->editor->is_edit_mode() ) {
				...
				echo esc_html__( 'No WooCommerce categories found. Add product categories or switch to manual mode.', 'luwi-emerald' );
				echo '</div>';
			}
			return;
		}
		...
		foreach ( $categories as $cat ) {
			...
			if ( $show_count ) {
				printf(
					...
					esc_html( sprintf( _n( '%d product', '%d products', $cat->count, 'luwi-emerald' ), $cat->count ) )
				);
			}
		...
		$this->render_luwipress_upsell(
			__( 'With LuwiPress: highlights top-opportunity categories from Knowledge Graph.', 'luwi-emerald' )
		);
	}
}
