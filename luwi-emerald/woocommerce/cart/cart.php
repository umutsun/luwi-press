	...
	<div class="luwi-cart-items">
		...
		<h2 class="luwi-cart-items__heading">
			...
			printf(
				...
				esc_html( _n( 'Your Cart (%d item)', 'Your Cart (%d items)', WC()->cart->get_cart_contents_count(), 'luwi-emerald' ) ),
						...
						<div class="luwi-cart-item__actions">
							...
							echo apply_filters(
								...
								sprintf(
									...
									esc_attr( sprintf( __( 'Remove %s from cart', 'luwi-emerald' ), wp_strip_all_tags( $_product->get_name() ) ) ),
									esc_attr( $product_id ),
		...
		<div class="luwi-cart-actions">
			<div class="luwi-cart-coupon coupon">
				<label for="coupon_code" class="screen-reader-text"><?php esc_html_e( 'Coupon:', 'luwi-emerald' ); ?></label>
				<input type="text" name="coupon_code" class="input-text" id="coupon_code" value="" placeholder="<?php esc_attr_e( 'Gift card or discount code', 'luwi-emerald' ); ?>" />
				<button type="submit" class="button<?php echo esc_attr( wc_wp_theme_get_element_class_name( 'button' ) ? ' ' . wc_wp_theme_get_element_class_name( 'button' ) : '' ); ?>" name="apply_coupon" value="<?php esc_attr_e( 'Apply', 'luwi-emerald' ); ?>"><?php esc_html_e( 'Apply', 'luwi-emerald' ); ?></button>
				<?php do_action( 'woocommerce_cart_coupon' ); ?>
			</div>

			<button type="submit" class="button<?php echo esc_attr( wc_wp_theme_get_element_class_name( 'button' ) ? ' ' . wc_wp_theme_get_element_class_name( 'button' ) : '' ); ?>" name="update_cart" value="<?php esc_attr_e( 'Update cart', 'luwi-emerald' ); ?>"><?php esc_html_e( 'Update cart', 'luwi-emerald' ); ?></button>
