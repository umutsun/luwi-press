...
if ( isset( $_POST['n8npress_trigger_translation'] ) && check_admin_referer( 'n8npress_translation_nonce' ) ) {
	...
	if ( ! empty( $lang ) && ! empty( $webhook_url ) ) {
		...
		$response = wp_remote_post( $url, array(
			'timeout' => 15,
			'headers' => array( 'Content-Type' => 'application/json', 'Authorization' => 'Bearer ' . get_option( 'n8npress_seo_api_token', '' ) ),
		...
		if ( ! is_wp_error( $response ) && $code >= 200 && $code < 300 ) {
			...
			echo '<div class="notice notice-success is-dismissible"><p>' . sprintf( __( 'Translation started: %s %s, up to %d items.', 'n8npress' ), strtoupper( $lang ), $type_label, $limit ) . '</p></div>';
...
if ( isset( $_POST['n8npress_trigger_taxonomy_translation'] ) && check_admin_referer( 'n8npress_translation_nonce' ) ) {
	...
	if ( ! empty( $tax_slug ) && ! empty( $webhook_url ) ) {
		...
		$response = wp_remote_post( $url, array(
			...
			'body'    => wp_json_encode( array(
				...
				'callback_url'     => get_site_url( null, '/wp-json/n8npress/v1/translation/taxonomy-callback' ),
				'api_token'        => get_option( 'n8npress_seo_api_token', '' ),
				'fetch_endpoint'   => get_site_url( null, '/wp-json/n8npress/v1/translation/taxonomy-missing' ),
			) ),
		) );
		$code = is_wp_error( $response ) ? 0 : wp_remote_retrieve_response_code( $response );
		if ( ! is_wp_error( $response ) && $code >= 200 && $code < 300 ) {
			$tax_label = $tax_types[ $tax_slug ] ?? $tax_slug;
			echo '<div class="notice notice-success is-dismissible"><p>' . sprintf( __( 'Taxonomy translation started: %s for all languages.', 'n8npress' ), esc_html( $tax_label ) ) . '</p></div>';
			N8nPress_Logger::log( 'Taxonomy translation triggered: ' . $tax_slug, 'info', array( 'taxonomy' => $tax_slug, 'languages' => $tax_languages ) );
		} else {
			$err = is_wp_error( $response ) ? $response->get_error_message() : 'HTTP ' . $code;
			echo '<div class="notice notice-error is-dismissible"><p>' . sprintf( __( 'Taxonomy translation failed: %s', 'n8npress' ), esc_html( $err ) ) . '</p></div>';
		}
	}
}
...
<div class="wrap n8n-tm">
	<h1><?php esc_html_e( 'Translation Manager', 'n8npress' ); ?></h1>
	...
	<div class="n8n-cards">
		<div class="n8n-card" style="border-left: 3px solid <?php echo $pct_color; ?>;">
			<div class="n8n-card-label"><?php esc_html_e( 'Overall Coverage', 'n8npress' ); ?></div>
		...
		<div class="n8n-card">
			<div class="n8n-card-label"><?php esc_html_e( 'Content Items', 'n8npress' ); ?></div>
			<div class="n8n-card-value"><?php echo $total_content; ?></div>
			<div class="n8n-card-sub"><?php echo count( $target_langs ); ?> <?php esc_html_e( 'languages', 'n8npress' ); ?></div>
		</div>
		<div class="n8n-card">
			<div class="n8n-card-label"><?php esc_html_e( 'Translated', 'n8npress' ); ?></div>
			<div class="n8n-card-value" style="color:#16a34a;"><?php echo $total_translated; ?></div>
			<div class="n8n-card-sub"><?php echo $total_possible; ?> <?php esc_html_e( 'needed', 'n8npress' ); ?></div>
		</div>
		<div class="n8n-card">
			<div class="n8n-card-label"><?php esc_html_e( 'Backend', 'n8npress' ); ?></div>
	...
	<div class="n8n-step" style="border-left: 3px solid <?php echo $tax_complete ? '#16a34a' : '#f59e0b'; ?>;">
		<div class="n8n-step-header">
			...
			<h2 class="n8n-step-title"><?php esc_html_e( 'Translate Taxonomies', 'n8npress' ); ?></h2>
			<?php if ( $tax_complete ) : ?>
				<span class="n8n-check"><?php esc_html_e( 'Complete', 'n8npress' ); ?></span>
			<?php endif; ?>
		</div>
		<?php if ( ! $tax_complete ) : ?>
		<div class="n8n-hint n8n-hint-warn"><?php esc_html_e( 'Translate categories and tags first so products are assigned to the correct translated categories.', 'n8npress' ); ?></div>
		<?php endif; ?>
		<div class="n8n-step-body">
			...
			<div class="n8n-tax-row">
				...
				<form method="post" style="display:inline;">
					...
					<button type="submit" name="n8npress_trigger_taxonomy_translation" class="button button-small button-primary">
						<?php printf( __( 'Translate All (%d)', 'n8npress' ), $tmiss ); ?>
					</button>
				</form>
				<?php elseif ( $tmiss === 0 ) : ?>
					<span class="n8n-check"><?php esc_html_e( 'Complete', 'n8npress' ); ?></span>
	...
	<div class="n8n-step" style="border-left: 3px solid <?php echo $all_done ? '#16a34a' : $data['color']; ?>;">
		<div class="n8n-step-header">
			...
			<span class="n8n-step-count"><?php echo $data['total']; ?> <?php esc_html_e( 'published', 'n8npress' ); ?></span>
			<?php if ( $all_done ) : ?>
				<span class="n8n-check"><?php esc_html_e( 'Complete', 'n8npress' ); ?></span>
			<?php endif; ?>
		</div>
		<?php if ( ! $tax_complete && $data['total'] > 0 ) : ?>
		<div class="n8n-hint n8n-hint-info"><?php esc_html_e( 'Complete Step 1 (Taxonomies) first for correct category assignments.', 'n8npress' ); ?></div>
		<?php endif; ?>
		<?php if ( $data['total'] === 0 ) : ?>
		<div class="n8n-step-body">
			<p style="color:#6b7280;margin:0;"><?php printf( __( 'No published %s found in the default language.', 'n8npress' ), strtolower( $data['label'] ) ); ?></p>
		</div>
		<?php else : ?>
		<table class="n8n-table">
			<thead><tr>
				<th><?php esc_html_e( 'Language', 'n8npress' ); ?></th>
				<th class="center" style="width:80px;"><?php esc_html_e( 'Done', 'n8npress' ); ?></th>
				<th class="center" style="width:80px;"><?php esc_html_e( 'Missing', 'n8npress' ); ?></th>
				<th style="width:200px;"><?php esc_html_e( 'Coverage', 'n8npress' ); ?></th>
			...
			<tr>
				...
				<td>
					...
					<form method="post" style="display:inline;">
						...
						<button type="submit" name="n8npress_trigger_translation" class="button button-primary button-small"><?php printf( __( 'Translate %d', 'n8npress' ), min( $missing, 20 ) ); ?></button>
					</form>
					<?php elseif ( $pct >= 100 ) : ?>
						<span class="n8n-check"><?php esc_html_e( 'Complete', 'n8npress' ); ?></span>
					<?php endif; ?>
	...
	<div class="n8n-step">
		<div class="n8n-step-header">
			<h2 class="n8n-step-title"><?php esc_html_e( 'Maintenance', 'n8npress' ); ?></h2>
		</div>
		<div class="n8n-tools" style="flex-direction:column;align-items:flex-start;gap:14px;">
			<div style="display:flex;gap:12px;align-items:center;flex-wrap:wrap;">
				<button type="button" id="n8npress-fix-categories" class="button button-primary"><?php esc_html_e( 'Fix Category Assignments', 'n8npress' ); ?></button>
				<span id="n8npress-fix-categories-result" style="font-size:13px;"></span>
				<span class="n8n-tools-hint"><?php esc_html_e( 'Re-assigns translated products to their correct translated categories.', 'n8npress' ); ?></span>
			</div>
			<div style="display:flex;gap:12px;align-items:center;flex-wrap:wrap;">
				<button type="button" id="n8npress-fix-images" class="button"><?php esc_html_e( 'Fix Translation Images', 'n8npress' ); ?></button>
				<span id="n8npress-fix-images-result" style="font-size:13px;"></span>
				<span class="n8n-tools-hint"><?php esc_html_e( 'Copies original product images to all translated products.', 'n8npress' ); ?></span>
			</div>
			<div style="display:flex;gap:12px;align-items:center;flex-wrap:wrap;">
				<button type="button" id="n8npress-clean-orphans" class="button" style="color:#dc2626;border-color:#dc2626;"><?php esc_html_e( 'Clean Orphan Translations', 'n8npress' ); ?></button>
				<span id="n8npress-clean-orphans-result" style="font-size:13px;"></span>
				<span class="n8n-tools-hint"><?php esc_html_e( 'Removes WPML translation records that have no matching original. Fixes inflated coverage counts.', 'n8npress' ); ?></span>
			</div>
		</div>
	</div>

	<script>
	document.getElementById('n8npress-fix-categories')?.addEventListener('click', function() {
		var btn = this, result = document.getElementById('n8npress-fix-categories-result');
		btn.disabled = true; btn.textContent = 'Fixing...';
		result.textContent = '';
		fetch(ajaxurl + '?action=n8npress_fix_category_assignments&nonce=<?php echo wp_create_nonce( 'n8npress_fix_categories' ); ?>', {method:'POST'})
			.then(function(r){return r.json()}).then(function(d){
				btn.disabled = false;
				btn.textContent = <?php echo wp_json_encode( __( 'Fix Category Assignments', 'n8npress' ) ); ?>;
	...
	document.getElementById('n8npress-fix-images')?.addEventListener('click', function() {
		...
		fetch(ajaxurl + '?action=n8npress_fix_translation_images&nonce=<?php echo wp_create_nonce( 'n8npress_fix_images' ); ?>', {method:'POST'})
			.then(function(r){return r.json()}).then(function(d){
				btn.disabled = false;
				btn.textContent = <?php echo wp_json_encode( __( 'Fix Translation Images', 'n8npress' ) ); ?>;
				result.textContent = d.success
					? d.data.fixed + ' fixed'
					: (d.data || 'Error');
				result.style.color = d.success ? '#16a34a' : '#dc2626';
	...
	document.getElementById('n8npress-clean-orphans')?.addEventListener('click', function() {
		if (!confirm(<?php echo wp_json_encode( __( 'This will delete orphan WPML translation records (terms and posts with no matching original). Affected content can be re-translated. Continue?', 'n8npress' ) ); ?>)) return;
		var btn = this, result = document.getElementById('n8npress-clean-orphans-result');
		btn.disabled = true; btn.textContent = <?php echo wp_json_encode( __( 'Cleaning...', 'n8npress' ) ); ?>;
		result.textContent = '';
		fetch(ajaxurl + '?action=n8npress_clean_orphan_translations&nonce=<?php echo wp_create_nonce( 'n8npress_clean_orphans' ); ?>', {method:'POST'})
			.then(function(r){return r.json()}).then(function(d){
				btn.disabled = false;
				btn.textContent = <?php echo wp_json_encode( __( 'Clean Orphan Translations', 'n8npress' ) ); ?>;
				if (d.success) {
