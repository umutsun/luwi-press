 ...
 *
 * Endpoint: GET /wp-json/n8npress/v1/site-config
 */
...
class N8nPress_Site_Config {
	...
	public function register_endpoints() {
		register_rest_route( 'n8npress/v1', '/site-config', array(
			'methods'             => 'GET',
			'callback'            => array( $this, 'get_site_config' ),
			'permission_callback' => array( $this, 'check_permission' ),
		) );

		register_rest_route( 'n8npress/v1', '/content/opportunities', array(
			'methods'             => 'GET',
	...
	public function get_site_config() {
		...
		$config = array(
			...
			'n8npress'    => $this->get_n8npress_info(),
		);
	...
	private function get_n8npress_info() {
		...
		$info = array(
			...
			'target_languages' => get_option( 'n8npress_translation_languages', array() ),
	...
	private function get_active_modules() {
		...
		if ( class_exists( 'N8nPress_AI_Content' ) ) {
			$modules[] = 'ai-content';
		}
		...
		if ( class_exists( 'N8nPress_Email_Proxy' ) ) {
			$modules[] = 'email-proxy';
		}
	...
	private function find_stale_content( $limit ) {
		$stale_days = absint( get_option( 'n8npress_freshness_stale_days', 90 ) );
