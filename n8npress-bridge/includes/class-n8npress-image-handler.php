...
class N8nPress_Image_Handler {
	...
	public static function generate_and_attach( $prompt, $post_id, array $options = array() ) {
		$provider = N8nPress_AI_Engine::get_provider( 'openai' );
		if ( is_wp_error( $provider ) ) {
			return $provider;
		}

		if ( ! ( $provider instanceof N8nPress_Provider_OpenAI ) ) {
			return new WP_Error( 'n8npress_wrong_provider', __( 'Image generation requires OpenAI provider.', 'n8npress' ) );
		}
	...
	public static function sideload_from_url( $url, $post_id = 0, $filename = '', $description = '' ) {
		...
		if ( is_wp_error( $tmp ) ) {
			return new WP_Error(
				'n8npress_download_failed',
				sprintf( __( 'Failed to download image: %s', 'n8npress' ), $tmp->get_error_message() )
			);
		}

		// Determine filename.
		if ( empty( $filename ) ) {
			$filename = 'n8npress-' . wp_generate_uuid4() . '.png';
		}
