 ...
 * filterable via apply_filters('n8npress_prompt_{workflow}', ...).
 *
 * @package N8nPress
 * @since   2.0.0
 */
...
class N8nPress_Prompts {
	...
	public static function product_enrichment( array $product, array $options = array() ) {
		$language   = $options['target_language'] ?? get_option( 'n8npress_target_language', 'English' );
...
Respond in JSON:
{
  ...
  "tags": ["suggested", "tags"]
		...
		return apply_filters( 'n8npress_prompt_product_enrichment', $prompt, $product, $options );
	}
...
Respond ONLY with valid JSON, no markdown.',
		...
		return apply_filters( 'n8npress_prompt_aeo_faq', $prompt, $product, $options );
	}
	...
	public static function review_response( array $review, $store_name = '', $language = '' ) {
		...
		if ( empty( $language ) ) {
			$language = get_option( 'n8npress_target_language', 'English' );
		}
...
Review: %4$s',
		...
		return apply_filters( 'n8npress_prompt_review_response', $prompt, $review, $store_name, $language );
	}
...
IMPORTANT: Return ONLY valid JSON, no markdown code blocks.',
		...
		return apply_filters( 'n8npress_prompt_content_generation', $prompt, $data );
	}
...
{"product_id": %1$d, "target_language": "%2$s", "title": "", "description": "", "short_description": "", "meta_title": "", "meta_description": "", "focus_keyword": "", "slug": ""}',
		...
		return apply_filters( 'n8npress_prompt_translation', $prompt, $content, $source_lang, $target_lang );
	}
...
[{"term_id": 123, "name": "translated name", "slug": "translated-slug", "language": "%3$s"}]',
		...
		return apply_filters( 'n8npress_prompt_taxonomy_translation', $prompt, $terms, $taxonomy, $source_lang, $target_lang );
	}
...
Store URL: %4$s',
		...
		return apply_filters( 'n8npress_prompt_internal_linking', $prompt, $post_data, $markers );
	}
...
- Include specific numbers when possible',
		...
		return apply_filters( 'n8npress_prompt_open_claw_chat', $prompt, $message, $site_context );
	}
	...
	public static function image_generation( $title, $context = '' ) {
		...
		return apply_filters( 'n8npress_prompt_image_generation', $prompt, $title, $context );
	}
}
